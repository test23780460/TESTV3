...
const serverEnvSchema = z.object({
  ...
  MARKET_DATA_PROVIDER: z.enum(["alpha_vantage", "finnhub", "coingecko"]).default("alpha_vantage"),
  NEWS_DATA_PROVIDER: z.enum(["finnhub", "alpha_vantage"]).default("finnhub"),
  CRON_SECRET: z.string().optional()
});

export type ServerEnv = z.infer<typeof serverEnvSchema>;
export function readServerEnv() { return serverEnvSchema.parse(process.env); }
...
export function redact(value: unknown) { return String(value ?? "").replace(/[A-Za-z0-9_-]{16,}/g, "[redacted]"); }
